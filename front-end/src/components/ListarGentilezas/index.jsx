...
class ListarGentilezas extends React.Component {
  ...
  renderizarIcones(gentileza) {
    ...
    return (
      <ul>
        ...
        <li className={classNameXP}>+{gentileza.obterXP()}xp</li>
      </ul>
    );
  }
  ...
  renderizarGentileza(gentileza, index) {
    ...
    if (
      this.props.uidGentileza === gentileza.uid &&
      !this.props.disableFadeout
    ) {
  ...
  renderizarTimestamp() {
    if (this.props.showTimeStamp && !this.props.disableFadeout) {
      ...
      if (this.state.fadeout) {
        className =
          "timestamp timestamp-fadeout pure-menu pure-menu-horizontal pure-menu-fixed";
      } else {
        className = "timestamp pure-menu pure-menu-horizontal pure-menu-fixed";
      }
      return <div className={className}>Atualizado em {timestamp}</div>;
    } else {
      return null;
    }
  }
  ...
  render() {
    let contador = 0;
    const lista = this.props.gentilezas
      .filter(entry => {
      ...
      .slice(0, this.props.totalExibicao)
      .map((entry, index) => this.renderizarGentileza(entry, index));
