...
class Usuario {
  constructor() {
    this.nome = "";
    this.genero = "";
    (this.avatar = Avatar.obterTodos()[0]), (this.gentilezas = []);
  }
  ...
  toString() {
    return `${this.nome}, ${this.avatar.toString()}, com ${
      this.gentilezas.length
    } gentileza(s)`;
  }
  ...
  excluirGentileza(uid, callback) {
    this.gentilezas.filter(gentileza => gentileza.uid === uid)[0].excluir();
    repository.salvar(this, callback);
  }
  executarGentileza(uid, callback) {
    this.gentilezas.filter(gentileza => gentileza.uid === uid)[0].executar();
    repository.salvar(this, callback);
  }
}
